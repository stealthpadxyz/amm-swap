...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    IBEP20 public stealth;
    uint256 public totalStakedAmount;
    // The PUNK TOKEN!
    address public stealthAddress;
    // PUNK tokens created per block.
    ...
    // The block number when PUNK mining starts.
    ...
    constructor(
        ...
        stealthAddress = address(_stealth);
        stealth = _stealth;
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = address(pool.lpToken) == address(stealthAddress) ? totalStakedAmount : pool.lpToken.balanceOf(address(this));
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = stealth.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBalance) {
            transferSuccess = stealth.transfer(_to, tokenBalance);
        } else {
            transferSuccess = stealth.transfer(_to, _amount);
        }
