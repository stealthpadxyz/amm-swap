...
export const SwitchUnits: React.FC = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"STEALTH" | "USD">("STEALTH");
  const conversionUnit = editingUnit === "STEALTH" ? "USD" : "STEALTH";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "STEALTH" ? "USD" : "STEALTH";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "STEALTH") {
      ...
      setValues({
        STEALTH: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
