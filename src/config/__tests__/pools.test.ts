...
describe('Config pools', () => {
  ...
  it.each(pools.map((pool) => [pool.sousId, pool.contractAddress]))(
    ...
    (sousId, contractAddress) => {
      const duplicates = pools.filter((p) => contractAddress[1] === p.contractAddress[1])
      expect(duplicates).toHaveLength(1)
    },
  )
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'ETH'))(
  ...
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'ETH'))(
