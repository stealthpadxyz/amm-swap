...
import { BSC_RPC_URLS, BSC_TESTNET_RPC_URLS, STEALTH_TESTNET_RPC_URLS } from '../config/constants/rpc'
...
export const setupNetwork = async (chainId?: number, externalProvider?: ExternalProvider) => {
  ...
  if (provider) {
    ...
    console.error("Can't setup the Shibarium network on metamask because window.ethereum is undefined")
    return false
  }
}
